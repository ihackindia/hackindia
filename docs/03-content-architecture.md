...
HackIndia is primarily a knowledge and discovery platform for hackathon builders. Its most important content type is the hackathon problem statement, supported by hackathon records, research, technical resources, and practical guides.
...
The content architecture should also preserve provenance. Official event information, HackIndia analysis, recommendations, and future community contributions must be distinguishable both in storage and in the rendered experience.

## 2. Problem Statement Content Model
...
- `existingSolutionsResearch`: Research into existing approaches, products, projects, and relevant findings.
...
The public slug is a routing and discovery identifier, not the only identity of a record. Internal `id` values and relationships should remain stable if a display title changes.
...
```text
Research -> Draft -> Review -> Publish -> Update -> Archive
```
...
```text
local structured content -> CMS/database -> API
```
...
Search is a future capability and should be designed around the structured fields already present in content records. Users should eventually be able to search and filter problems by:
...
The first implementation can search local structured records in a server-side route or application layer. At larger scale, the same normalized fields can feed a dedicated search index or database query. Search results should link to canonical detail pages and should not become a second source of truth for content.

No search functionality is implemented by this document.

## 11. Google Search Strategy

Each substantial problem statement should have an independently indexable page because builders are likely to search for an identifier, title, solution idea, prototype, technology stack, or implementation guidance directly. A canonical page such as `/problems/sih26001` can answer that intent with structured, relevant content.
...
## 13. Future Community Contributions
